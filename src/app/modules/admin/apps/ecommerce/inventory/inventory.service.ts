...
export class InventoryService
{
     ...
     * @param search
     */
    getProducts(page: number = 0, size: number = 10, sort: string = 'name', order: 'asc' | 'desc' | '' = 'asc', searchValue: string = ''): Observable<{ data: InventoryPagination }>
    {

        return this._httpClient.post<{ data: InventoryPagination }>(`${environment.baseUrl}admin/inventory/get_all`, {
                page: '' + (page + 1),
                limit: '' + size,
                sort,
                order,
    ...
    getProductById(id: string): Observable<InventoryProduct>
    {
        return this._products.pipe(
            ...
            map((products) => {
                ...
                // Find the product
                const product = products.find(item => item.code === id) || null;
    ...
    createProduct(): Observable<InventoryProduct>
    {
        return this.products$.pipe(
            take(1),
            switchMap(products => this._httpClient.post<{data : InventoryProduct}>(`${environment.baseUrl}admin/inventory/create`,
                {
                                    code         : FuseMockApiUtils.guid(),
                                    name         : 'A New Product',
                }).pipe(
                map((newProduct) => {
     ...
     * @param product
     */
    updateProduct(code: string, product: InventoryProduct): Observable<InventoryProduct>
    {
        return this.products$.pipe(
            take(1),
            switchMap(products => this._httpClient.post<{data : InventoryProduct}>(`${environment.baseUrl}admin/inventory/update`, {
                code,
                product
            }).pipe(
                map((updatedProduct) => {
    ...
    deleteProduct(code: string): Observable<boolean>
    {
        return this.products$.pipe(
            take(1),
            switchMap(products => this._httpClient.post(`${environment.baseUrl}admin/inventory/delete`, {
                code
            }).pipe(
                        map(() => {
        ...
        return this.contacts$.pipe(
            ...
            switchMap(contacts => this._httpClient.post<Contact>('api/apps/contacts/avatar', {
                ...
                map((updatedContact) => {
                    ...
                    const index = contacts.findIndex(item => item.code === id);
